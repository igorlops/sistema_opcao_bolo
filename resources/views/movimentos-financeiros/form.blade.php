<div class="form-group row {{ $errors->has('descricao') ? 'has-error' : ''}}">
    ...
    <label for="empresa_id" class="col-form-label col-sm-2 required">{{ 'Empresa*' }}</label>
    <div class="col-sm-10">
        <select class="form-control" name="empresa_id" id="empresa-ajax" required >
            @if (isset($movimentos_financeiro))
                <option value="{{$movimentos_financeiro->empresa_id}}">{{$movimentos_financeiro->empresa->nome}} ({{$movimentos_financeiro->empresa->razao_social}})</option>
            @endif
        </select>
