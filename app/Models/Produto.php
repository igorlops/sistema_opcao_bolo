    ...
    protected $fillable = ['nome', 'is_bolo_extra','tipo_produto'];
     ...
     public function relacaoProdutosProducao($data_ini,$data_fin, $user_id = null) {
        ...
        $produtos = $this->select('produtos.id', 'produtos.nome')
            ->selectRaw("COALESCE((SELECT SUM(estoques.quantidade) FROM estoques WHERE tipo_estoque = 'd' AND created_at BETWEEN ? AND ? AND id_produto = produtos.id  AND produtos.tipo_produto = 'p' " . ($user_id ? "AND estoques.user_id = ".$user_id : '') . "), 0) AS desperdicio", [$data_ini. ' 00:00:00', $data_fin. ' 23:59:59'])
            ->selectRaw("COALESCE((SELECT SUM(estoques.quantidade) FROM estoques WHERE tipo_estoque = 'p' AND created_at BETWEEN ? AND ? AND id_produto = produtos.id  AND produtos.tipo_produto = 'p' " . ($user_id ? "AND estoques.user_id = ".$user_id : '') . "), 0) AS producao",  [$data_ini. ' 00:00:00', $data_fin. ' 23:59:59'])
            ->selectRaw("COALESCE((SELECT COUNT(*) FROM entradas WHERE created_at BETWEEN ? AND ? AND entradas.id_produto = produtos.id  AND produtos.tipo_produto = 'p' " . ($user_id ? "AND entradas.user_id = ".$user_id : '') . " AND entradas.metade IS NULL), 0) AS vendaCompleta", [$data_ini. ' 00:00:00', $data_fin. ' 23:59:59'])
            ->selectRaw("COALESCE((SELECT COUNT(*) FROM entradas WHERE created_at BETWEEN ? AND ? AND entradas.id_produto = produtos.id  AND produtos.tipo_produto = 'p' " . ($user_id ? "AND entradas.user_id = ".$user_id : '') . " AND entradas.metade LIKE '%on%'), 0) AS vendaMetade", [$data_ini. ' 00:00:00', $data_fin. ' 23:59:59'])
